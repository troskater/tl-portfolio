...
const getConfigValue = () => {
  ...
  if (process.env.NODE_ENV == "production" && firebaseRemoteConfig) {
    fetchAndActivate(firebaseRemoteConfig).then(() => {
      return {
        password: getValue(firebaseRemoteConfig, "redis_password"),
        socket: {
          host: getValue(firebaseRemoteConfig, "redis_uri"),
          port: getValue(firebaseRemoteConfig, "redis_port")
        }
      }
    }
    )
    ...
    return {
      url: process.env.RedisDevUri
    }
  }
};

// connect to client
const redis = await createClient(getConfigValue())
